...
must_haves:
  truths:
    ...
    - "POST /api/rewards/enroll creates reward_address record"
  artifacts:
...
<objective>
Create backend infrastructure for auth integration: admin identification via ADMIN_USER_IDS, enrollment status helpers, and rewards API routes.

Purpose: Enable the frontend to check admin status, enrollment status, and allow users to enroll in the rewards program.

Output: Admin utilities, requireAdmin middleware, enrollment helpers, and /api/rewards/* endpoints.
</objective>
